...
def check_method_consistency(report: RCAReport) -> list[CritiqueIssue]:
    ...
    if report.method == "fishbone":
        fishbone = detail.fishbone if detail else None
        if fishbone is None:
            return [
                CritiqueIssue(
        ...
        selected_category = fishbone.selected_category
        selected_cause = fishbone.selected_cause
        ...
        if _normalized_phrase(report.root_cause) != _normalized_phrase(selected_cause):
            issues.append(
                CritiqueIssue(
                    check="method_consistency",
                    severity="high",
    ...
    if report.method == "fault_tree":
        fault_tree = detail.fault_tree if detail else None
        ...
        gates = fault_tree.gates
        basic_causes = fault_tree.basic_causes
